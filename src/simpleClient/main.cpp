...
void createWindow()
{
	...
	//create window
	win = SDL_CreateWindow(exeNameCStr, 800, 100, 600, 600, SDL_WINDOW_OPENGL); //same height and width makes the window square ...
...
void postRender()
{
	...
	if (singleLine) cout << "                                                                                \r";
	cout << frameLine << std::flush;
	if (! singleLine) cout << std::endl;
...
void handleNetwork()
{
	...
	if (gotMessage)
	{
		...
		frameLine += "Message received: \"" + std::string(the_data) + "\" ";

		std::istringstream iss(static_cast<char*>(update.data()));
		iss >> position1.x >> position1.y >> position2.x >> position2.y;

		frameLine += "x, y: " + std::to_string(position1.x) + ", " + std::to_string(position1.y) + " ";
	}
	else
	{
		frameLine += "no data. ";
