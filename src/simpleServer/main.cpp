...
void resetPositions()
{
	position1 = glm::vec3(-0.5f, -0.5f, 0.0f);
	position2 = glm::vec3(0.8f, -0.5f , 0.0f);
}
...
void updateNetwork()
{
	...
	snprintf ((char *) message.data(), 48 ,
		"%05f %05f %05f %05f", position1.x, position1.y, position2.x, position2.y);
	frameLine += "Message sent: \"" + std::string(static_cast<char*>(message.data()), message.size()) + "\" ";
